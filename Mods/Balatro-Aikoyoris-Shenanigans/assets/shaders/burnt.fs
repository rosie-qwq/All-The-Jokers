...
float randomize(in vec3 px){
    return fract(fract(sin(px.x*0.123412)*cos(px.y*0.1325))*1123.412*35.231258*sin(px.z*0.5125));
}
...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
    ...
    /*
    tex.x *= 0.9 + 0.12 * sin(burnt.y);
    tex.y *= 0.8 + 0.21 * sin(burnt.y*1.123);
    tex.z *= 0.7 + 0.05 * sin(burnt.y*5.1234);
    */
    //tex.w *= rand;
